...
class tutorial03Turb : public SteadyNSTurb
{
    public:
        /// Constructor
        explicit tutorial03Turb(int argc, char* argv[])
            :
...
int main(int argc, char* argv[])
{
 ...
 Info << "Matrices finished "  << endl;
    // Create the reduced object
    ReducedSteadyNSTurb ridotto(example);
    ...
    rbfCoeff.resize(NmodesNUTproj, example.mu.size());
 ...
 Info << "Here loop "  << endl;
        ...
        ridotto.solveOnlineSUP(vel_now);
        rbfCoeff.col(k) = ridotto.rbfCoeff;
    ...
    ridotto.reconstructSUP("./ITHACAoutput/Reconstruction/");
    return 0;
}
