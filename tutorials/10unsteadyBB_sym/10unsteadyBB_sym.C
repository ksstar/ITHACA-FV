...
class tutorial101: public unsteadyBB
{
    public:
        explicit tutorial101(int argc, char* argv[])
            :
        ...
        void offlineSolve(Eigen::MatrixXd par_BC)
        {
		    ...
		    scalar p0 = 0;
		    assignIF(p, p0);
			//for (label l = 0; l < p.boundaryField().size(); l++)
                	//{
                    	//	assignBC(p,l,p0);
                	//}
                    truthSolve(mu_now);
...
int main(int argc, char* argv[])
{
    ...
    example.inletIndexT << 0;
    /// Time parameters
    example.startTime = 0.0;
    example.finalTime = 1000.0;
    example.timeStep = 1;
    example.writeEvery = 1;
    ...
    {
        Eigen::MatrixXd temp_now_BC(1, 1);
    ...
    tutorial101 HFonline2(argc, argv);
    HFonline2.Pnumber = 1;
    HFonline2.Tnumber = 1;
    ...
    tutorial101 HFonline3(argc, argv);
    HFonline3.Pnumber = 1;
    HFonline3.Tnumber = 1;
