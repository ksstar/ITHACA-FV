...
class tutorial19: public unsteadyNS
{
    public:
...
int main(int argc, char* argv[])
{
    ...
    word par_offline_BCp("./timeBCpoff");
    Eigen::MatrixXd par_on_BC;
    word par_online_BC("./timeBCon");
    par_on_BC = ITHACAstream::readMatrix(par_online_BC);
    Eigen::MatrixXd par_on_BCp;
    word par_online_BCp("./timeBCpon");
    par_on_BCp = ITHACAstream::readMatrix(par_online_BCp);
    ...
    example.inletIndexP.resize(1, 1); // rows: total number of patches
    example.inletIndexP(0, 0) = 1;  // Patch 1 - Inlet

    // Time parameters
    example.startTime = 0;
    example.finalTime = 1000;
    example.timeStep = 2;
    example.writeEvery = 4;
    ...
    reduced.solveOnline_PPE(vel_now, pres_now);
