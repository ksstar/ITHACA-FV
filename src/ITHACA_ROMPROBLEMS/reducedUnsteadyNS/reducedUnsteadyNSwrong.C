...
std::cout << "Here B4" << std::endl;
    ...
    online_solution.resize(onlineSize + 1 );
                                        ...
                                        label startSnap)
{
    ...
    while (diffvel.maxCoeff()> tolerancePenalty && Iter < maxIterPenalty)
    {
        ...
        y.resize(Nphi_u + Nphi_p, 1);
        y.setZero();
        y.head(Nphi_u) = ITHACAutilities::get_coeffs(problem->Ufield[0],Umodes);
        y.tail(Nphi_p) = ITHACAutilities::get_coeffs(problem->Pfield[0],Pmodes);
