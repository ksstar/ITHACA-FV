...
/// \file
/// Source file of the SteadyBB class.
...
// Constructor
SteadyBB::SteadyBB() {}
SteadyBB::SteadyBB(int argc, char* argv[])
{
...
void SteadyBB::projectSUP(fileName folder, label NU, label NP, label NT,
                            label NSUP)
{
    NUmodes = NU;
    NTmodes = NT;
...
// * * * * * * * * * * * * * * Momentum Eq. Methods * * * * * * * * * * * * * //
Eigen::MatrixXd SteadyBB::pressure_gradient_term(label NUmodes,
        label NPrghmodes,
        label NSUPmodes)
{
...
Eigen::MatrixXd SteadyBB::divergence_term(label NUmodes, label NPrghmodes,
        label NSUPmodes)
{
...
Eigen::MatrixXd SteadyBB::buoyant_term_poisson(label NPrghmodes,
        label NTmodes)
{
    ...
    for (label i = 0; i < H1size; i++)
    {
        for (label j = 0; j < H2size; j++)
        {
            HP_matrix(i, j) = fvc::domainIntegrate(fvc::reconstruct(fvc::snGrad(
                    Prghmodes[i]) * Prghmodes[i].mesh().magSf())  &fvc::reconstruct(
...
// * * * * * * * * * * * * * * Energy Eq. Methods * * * * * * * * * * * * * //
List<Eigen::MatrixXd> SteadyBB::convective_term_temperature(label NUmodes,
...
Eigen::MatrixXd SteadyBB::diffusive_term_temperature(label NUmodes,
        label NTmodes, label NSUPmodes)
{
...
Eigen::MatrixXd SteadyBB::mass_term_temperature(label NUmodes, label NTmodes,
        label NSUPmodes)
{
...
void SteadyBB::restart()
{
    volScalarField& p = _p();
    volScalarField& p_rgh = _p_rgh();
    volScalarField& p0 = _p0();
    volScalarField& p0_rgh = _p0_rgh();
    volVectorField& U = _U();
